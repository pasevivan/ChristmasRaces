...
public class ControllerImpl implements Controller {
    ...
    public String startRace(String raceName) {
       ...
       winners
                .entrySet()
                .stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).forEachOrdered(e -> sortedDrivers.add(e.getKey()));

//        String firstKey = sortedMap.keySet().iterator().next();
//        sortedMap.remove(firstKey);
//        String secondKey = sortedMap.keySet().iterator().next();
//        sortedMap.remove(secondKey);
//        String thirdKey = sortedMap.keySet().iterator().next();

        return String.format("Driver %s wins %s race.", sortedDrivers.get(0), raceName) +
                System.lineSeparator() +
                String.format("Driver %s is second in %s race.", sortedDrivers.get(1), raceName) +
                System.lineSeparator() +
                String.format("Driver %s is third in %s race.", sortedDrivers.get(2), raceName);
